...
* */
  ...
  * */
  ...
  test("Test new control flow graph for IfElseIF stmt final function"){
    ...
    val firstElifStmt = ElseIfStmt(GTExpression(IntValue(1), IntValue(2)), thirdAssignmentStmt)
    val secondElifStmt = ElseIfStmt(LTExpression(IntValue(4), IntValue(5)), fourthAssignmentStmt)
    val thirdElifStmt = ElseIfStmt(GTExpression(IntValue(10), IntValue(5)), fifthAssignmentStmt)

    val elseIf : List[ElseIfStmt] = List(firstElifStmt, secondElifStmt, thirdElifStmt)

    val ifElif = IfElseIfStmt(firstExpression, firstAssignmentStmt, elseIf, secondAssignmentStmt)

    val grafo = new NewControlFlowGraph()
    val teste = grafo.finalFG(ifElif)

    assert(teste == Set(firstAssignmentStmt, thirdAssignmentStmt, fourthAssignmentStmt, fifthAssignmentStmt,
  ...
  test("Test new control flow graph for while stmt final function"){
    ...
    val whileStmt = WhileStmt(GTExpression(IntValue(3), IntValue(5)), SequenceStmt(sequenceStmts))
  ...
  test("Test new control flow graph for case stmt final function"){
    val firstAssignmentStmt = AssignmentStmt("x", IntValue(2))
    val secondAssignmentStmt = AssignmentStmt("y", IntValue(3))
    val thirdAssignmentStmt = SequenceStmt(List(firstAssignmentStmt, secondAssignmentStmt))
    ...
    val cases : List[CaseAlternative] = List(firstCase, secondCase, thirdCase, fourthCase, fifthCase)
    ...
    assert(caseStmtFinalTest == Set(firstAssignmentStmt, secondAssignmentStmt, secondAssignmentStmt,
      fourthAssignmentStmt, fifthAssignmentStmt, sixthAssignmentStmt))
  ...
  test("Test new control flow graph for ifelse stmt flow function"){
    ...
    val ifElse = IfElseStmt(GTEExpression(IntValue(2), IntValue(5)), firstAssignmentStmt, secondAssignmentStmt)

    val grafo = new NewControlFlowGraph()
    val ifElseStmtFlowTest = grafo.flow(ifElse)

    assert(ifElseStmtFlowTest == Set((ifElse, firstAssignmentStmt), (ifElse, secondAssignmentStmt.get)))
  }

  test("Test new control flow graph for while stmt flow function"){
    val assignmentStmt = AssignmentStmt("x", IntValue(2))

    val whileStmt = WhileStmt(GTExpression(IntValue(3), IntValue(5)), assignmentStmt)

    val grafo = new NewControlFlowGraph()
    val whileStmtFlowTest = grafo.flow(whileStmt)

    assert(whileStmtFlowTest == Set((whileStmt, assignmentStmt), (assignmentStmt, whileStmt)))
  }
  ...
  test("Test new control flow graph for sequence stmt flow function - Only Assignments"){
    ...
    assert(sequenceStmtFlowTeste == Set((firstAssignmentStmt, secondAssignmentStmt),
  ...
  test("Test new control flow graph for sequence stmt flow function"){
    ...
    val ifElse = IfElseStmt(GTEExpression(IntValue(2), IntValue(5)), thenIfStmt, elseIfStmt)
    ...
    assert(sequenceStmtFlowTeste == Set((firstAssignmentStmt, secondAssignmentStmt),
      (secondAssignmentStmt, thirdAssignmentStmt), (thirdAssignmentStmt, fourthAssignmentStmt), (ifElse, thenIfStmt),
      (ifElse, elseIfStmt.get), (fourthAssignmentStmt, ifElse)))
