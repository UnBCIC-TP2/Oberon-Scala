...
test("Test control flow graph RepeatUntilStmt 02 - 1 Expression and 1 Condition ") {
  ...
  test("Test control flow graph RepeatUntilStmt 04 - 1 Expression and 1 Condition ") {
    val stmt0 = AssignmentStmt("x", IntValue(50))
    val stmt1 = AssignmentStmt("x", SubExpression(VarExpression("x"), IntValue(1)))
    val stmt2 = RepeatUntilStmt(GTExpression(VarExpression("x"), IntValue(0)), stmt1)
    val stmt3 = WriteStmt(VarExpression ("x"))
