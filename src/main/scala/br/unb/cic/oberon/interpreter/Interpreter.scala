...
/** The interpreter visitor first updates the environment with the global
  * constants, variables, and procedures; after that, it visits the main program
  * statement.
  *
  * It uses an additional visitor (EvalExpressionVisitor) to compute the value
  * of the different expressions.
  *
  * We assume the program is well-typed, otherwise, a runtime exception might be
  * thrown.
  */
class Interpreter extends OberonVisitorAdapter {
  ...
  override def visit(variable: VariableDeclaration): Unit = {
    env.baseType(variable.variableType) match {
      case Some(ArrayType(length, baseType)) =>
        env.setGlobalVariable(
          variable.name,
          ArrayValue(
            ListBuffer.fill(length)(Undef()),
            ArrayType(length, baseType)
          )
        )
      ...
      case AssignmentStmt(designator, exp) =>
        designator match {
          case ArrayAssignment(array, index) =>
            visitArrayAssignment(array, index, exp)
          // TODO:
...
//               stmts.accept(this)
  ...
  private def checkIfElseIfStmt(
      condition: Expression,
      thenStmt: Statement,
      listOfElseIf: List[ElseIfStmt],
    ...
    if (evalCondition(condition)) thenStmt.accept(this)
      ...
      while (i < listOfElseIf.size && !matched) {
        listOfElseIf(i) match {
          case ElseIfStmt(condition, stmt) =>
  ...
  def updateEnvironmentWithProcedureCall(
      procedure: Procedure,
      args: List[Expression]
  ): Unit = {
    val mappedArgs = procedure.args
      .zip(args)
      .map(pair =>
        pair match {
          case (ParameterByReference(_, _), VarExpression(name2)) =>
            (pair._1, env.pointsTo(name2))
    ...
    mappedArgs.foreach(pair =>
      pair match {
        case (ParameterByReference(name, _), Some(location: Location)) =>
          env.setParameterReference(name, location)
        case (ParameterByReference(_, _), _) => throw new RuntimeException
        case (ParameterByValue(name, _), exp: Expression) =>
...
class EvalExpressionVisitor(val interpreter: Interpreter)
    extends OberonVisitorAdapter {
  type T = Expression
  ...
  override def visit(exp: Expression): Expression = exp match {
    ...
    case VarExpression(name) =>
      ...
      if (variable.isEmpty)
        throw new NoSuchElementException(f"Variable $name is not defined")
      interpreter.env.lookup(name).get
    case ArraySubscript(a, i) =>
      visitArraySubscriptExpression(ArraySubscript(a, i))
    case AddExpression(left, right) =>
      arithmeticExpression(left, right, (v1: Number, v2: Number) => v1 + v2)
    ...
    case ModExpression(left, right) =>
      modularExpression(left, right, (v1: Modular, v2: Modular) => v1.mod(v2))
    case EQExpression(left, right) =>
      binExpression(left, right, (v1: Value, v2: Value) => BoolValue(v1 == v2))
    ...
    case NotExpression(exp) =>
      BoolValue(
        !exp.accept(this).asInstanceOf[Value].value.asInstanceOf[Boolean]
      )
    case AndExpression(left, right) =>
      binExpression(
        ...
        (v1: Value, v2: Value) =>
          BoolValue(
            v1.value.asInstanceOf[Boolean] && v2.value.asInstanceOf[Boolean]
          )
      )
  ...
  def visitArraySubscriptExpression(
  ...
  def visitFunctionCall(name: String, args: List[Expression]): Expression = {
    ...
    assert(
      returnValue.isDefined
    ) // a function call must set a local variable with the "return" expression
    returnValue.get
  }
  ...
  /** Eval an arithmetic expression on Numbers
    ...
    * @return
    *   the application of the operator after evaluating left and right to
  ...
  /** Eval an modular expression on Numbers
    ...
    *   reduce them to numbers.
    */
  def modularExpression(
      left: Expression,
      right: Expression,
      fn: (Modular, Modular) => Modular
  ): Expression = {
  ...
  /** Eval a binary expression on values.
    ...
    *   the right expression
    * @param fn
    *   a function that constructs an expression. Here we are using a high-order
    *   function. We assign to the "result" visitor attribute the value we
    *   compute after applying this function.
    */
  def binExpression(
      left: Expression,
      right: Expression,
      fn: (Value, Value) => Expression
  ): Expression = {
