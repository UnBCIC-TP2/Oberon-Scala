...
class Interpreter {
  ...
  def runInterpreter(module: OberonModule, select: String = "BASE"): Environment[Expression] = {
    ...
    val env5 = module.tests.foldLeft(env4)((a, b) => execTest(a, b))

    envt = env5

    // executes test statements if test interpreter has been selected
    if (select == "TEST" || select == "BOTH")
      module.tests.foreach(t => if (t.modifier == "TEST") callTest(t.name, envt))

    // execute the statement if it is defined.
    // remember, module.stmt is an Option[Statement].
    if (module.stmt.isDefined && (select == "BASE" || select == "BOTH")) {
  ...
  def execTest(environment: Environment[Expression], test: Test): Environment[Expression] = {
  ...
  def callTest(name: String, environment: Environment[Expression]): Environment[Expression] = {
    val test = env.findTest(name)
    val env1 = updateEnvironmentWithTest(test, environment)
    execStatement(env1, test.stmt)
  }
