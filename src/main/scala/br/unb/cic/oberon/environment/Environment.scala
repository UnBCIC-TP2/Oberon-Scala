...
class Environment[T](
    ...
    private val stack: Stack[Map[String, Location]] =
      Stack.empty[Map[String, Location]],
    private val procedures: Map[String, Procedure] =
      Map.empty[String, Procedure],
    private val tests: Map[String, Test] = Map.empty[String, Test],
    private val userDefinedTypes: Map[String, UserDefinedType] =
      Map.empty[String, UserDefinedType]
) {

  def setGlobalVariable(name: String, value: T): Environment[T] = {
    return new Environment[T](
      top_loc = this.top_loc + 1,
  ...
  def addUserDefinedType(userType: UserDefinedType): Environment[T] = {
    new Environment[T](
      ...
      procedures = this.procedures,
      tests = this.tests,
      userDefinedTypes = this.userDefinedTypes + (this
        .userDefinedTypeName(userType) -> userType),
      stack = this.stack
    )
  }
  ...
  def lookupProcedure(name: String): Option[Procedure] = {
    if (procedures.contains(name)) {
      Some(procedures(name))
    } else {
      None
    }
  }
