...
/** A parser for the Oberon language using the ANTLR infrastructure. The main
  * idea of this parser is to parse a Oberon source code and convert it into our
  * Oberon AST representation in Scala. Here we use the ANTLR idiom for parsing
  * source code.
  *
  * @author
  *   rbonifacio
...
class ParserVisitor {
  ...
  def visitCompilationUnit(ctx: OberonParser.CompilationUnitContext): Unit = {
    ...
    val constants =
      ctx.declarations().constant().asScala.toList.map(c => visitConstant(c))
    variables = ctx
      .declarations()
      .varDeclaration()
      .asScala
      .toList
      .flatMap(v => visitVariableDeclaration(v))
    val procedures = ctx
      .declarations()
      .procedure()
      .asScala
      .toList
      .map(p => visitProcedureDeclaration(p))
    val userTypes = ctx
      .declarations()
      .userTypeDeclaration()
      .asScala
  ...
  /** Visit a block declaration. If the block context is null, we return None.
    * Else, we return a statement with the block statement.
    *
    * @param ctx
    *   block statement of an Oberon module
    * @return
  ...
  /** @brief
    *   Qualify takes a qualified name and returns a string representation of
    *   it. For instance, if we are in a module A and try to qualify a variable
    *   x, this method would return "A::x" as that literal string.
    */
  ...
  /** Visit a constant declaration.
    *
    * @param ctx
    *   constant declaration visited
    * @return
    *   a constant declaration representation
    */
  ...
  /** Visit a variable declaration
    ...
    * @return
    *   a variable declaration representation
    */
  def visitVariableDeclaration(
      ctx: OberonParser.VarDeclarationContext
  ): List[VariableDeclaration] = {
    val variableType = visitOberonType(ctx.varType)
    ctx.vars.asScala.toList.map(v =>
      VariableDeclaration(v.getText, variableType)
    )
  }
  ...
  def visitProcedureDeclaration(
    ...
    val args = ctx
      ...
      .asScala
      .toList
      .flatMap(formal => visitFormalArg(formal))
    val constants =
      ctx.declarations().constant().asScala.toList.map(c => visitConstant(c))
    val variables = ctx
      .declarations()
      ...
      .asScala
      .toList
      .map(v => visitVariableDeclaration(v))
      .flatten
    val block = visitModuleBlock(ctx.block())

    val returnType =
      if (ctx.procedureType != null) Some(visitOberonType(ctx.procedureType))
      else None
  ...
  class FormalArgVisitor extends OberonBaseVisitor[List[FormalArg]] {
    override def visitParameterByValue(
        ctx: OberonParser.ParameterByValueContext
    ): List[FormalArg] = {
      val argType = visitOberonType(ctx.argType)
      ctx.Id().asScala.toList.map(arg => ParameterByValue(arg.getText, argType))
    }

    override def visitParameterByReference(
        ctx: OberonParser.ParameterByReferenceContext
    ): List[FormalArg] = {
      val argType = visitOberonType(ctx.argType)
      ctx
        .Id()
        .asScala
        .toList
        .map(arg => ParameterByReference(arg.getText, argType))
    }
  }

  def visitUserDefinedType(
      ctx: OberonParser.UserTypeDeclarationContext
  ): UserDefinedType = {
  ...
  /** Visit an oberon type.
    *
    * Note: this implementation uses pattern matching as an expression.
    *
    * @param ctx
    *   the oberon type context
    * @return
    *   a oberon type representation.
    */
  ...
  class OberonTypeVisitor extends OberonBaseVisitor[Unit] {
    ...
    override def visitBooleanType(
        ctx: OberonParser.BooleanTypeContext
    ): Unit = {
      baseType = BooleanType
    }

    override def visitCharacterType(
        ctx: OberonParser.CharacterTypeContext
    ): Unit = {
    ...
    override def visitReferenceType(
        ctx: OberonParser.ReferenceTypeContext
    ): Unit = {
    ...
    override def visitComplexType(
        ctx: OberonParser.ComplexTypeContext
    ): Unit = {
  ...
  def visitUserType(ctx: OberonParser.UserTypeContext): Type = {
    ...
    if (userTypeVisitor.baseType == null) UndefinedType
    else userTypeVisitor.baseType
  }

  class UserTypeVisitor extends OberonBaseVisitor[Unit] {
    var baseType: Type = _

    override def visitLambdaTypeDeclaration(
        ctx: OberonParser.LambdaTypeDeclarationContext
    ): Unit = {
      ...
      val args =
        ctx
          ...
          .asScala
          .toList
          .map(t => visitOberonType(t))
      baseType = LambdaType(args, returnType)
    }

    override def visitArrayTypeDeclaration(
        ctx: OberonParser.ArrayTypeDeclarationContext
    ): Unit = {
    ...
    override def visitRecordTypeDeclaration(
        ctx: OberonParser.RecordTypeDeclarationContext
    ): Unit = {
    ...
    override def visitPointerTypeDeclaration(
        ctx: OberonParser.PointerTypeDeclarationContext
    ): Unit = {
      val typeVisitor = new ParserVisitor()
  ...
  class REPLVisitor() extends OberonBaseVisitor[Unit] {
    ...
    override def visitREPLVarDeclaration(
        ctx: OberonParser.REPLVarDeclarationContext
    ): Unit = {
    ...
    override def visitREPLConstant(
        ctx: OberonParser.REPLConstantContext
    ): Unit = {
      val c = ctx.constant()
    ...
    override def visitREPLStatement(
        ctx: OberonParser.REPLStatementContext
    ): Unit = {
      val s = ctx.statement()
      val stmtVisitor = new StatementVisitor
    ...
    override def visitREPLExpression(
        ctx: OberonParser.REPLExpressionContext
    ): Unit = {
    ...
    override def visitREPLUserTypeDeclaration(
        ctx: OberonParser.REPLUserTypeDeclarationContext
    ): Unit = {
  ...
  class ExpressionVisitor() extends OberonBaseVisitor[Unit] {
    ...
    override def visitStringValue(
        ctx: OberonParser.StringValueContext
    ): Unit = {
    ...
    override def visitFieldAccess(
        ctx: OberonParser.FieldAccessContext
    ): Unit = {
    ...
    override def visitArraySubscript(
        ctx: OberonParser.ArraySubscriptContext
    ): Unit = {
    ...
    override def visitAddExpression(
        ctx: OberonParser.AddExpressionContext
    ): Unit =
      visitBinExpression(ctx.left, ctx.right, expression(ctx.opr.getText))

    override def visitMultExpression(
        ctx: OberonParser.MultExpressionContext
    ): Unit =
      visitBinExpression(ctx.left, ctx.right, expression(ctx.opr.getText))
    ...
    override def visitFunctionCall(
        ctx: OberonParser.FunctionCallContext
    ): Unit = {
      val name = ctx.name.getText
    ...
    override def visitNotExpression(
        ctx: OberonParser.NotExpressionContext
    ): Unit = {
      ctx.exp.accept(this)
    ...
    // ): Unit = {
    //   val listExpressions = new ListBuffer[Expression]
    //   ctx
    //      .expression()
    //      .listExpressions()
    //      .forEach(e => {
    ...
    //   val visitor = new ExpressionVisitor()
    //   ctx.expression.accept(visitor)
    //   val listExp = listExpressions.toList
     ...
     */
    private def visitBinExpression(
        left: OberonParser.ExpressionContext,
        right: OberonParser.ExpressionContext,
      ...
      exp = constructor(
        lhs,
        rhs
      ) // assign the result to exp, using the 'constructor' to set the actual expression
      ()
    }
  }
  ...
  class StatementVisitor extends OberonBaseVisitor[Unit] {
    ...
    override def visitAssignmentStmt(
        ctx: OberonParser.AssignmentStmtContext
    ): Unit = {
      val visitor = new ExpressionVisitor()
    ...
    override def visitSequenceStmt(
        ctx: OberonParser.SequenceStmtContext
    ): Unit = {
    ...
    def flatSequenceOfStatements(stmts: List[Statement]): List[Statement] =
      stmts match {
        case SequenceStmt(ss) :: rest =>
          flatSequenceOfStatements(ss) ++ flatSequenceOfStatements(rest)
        case s :: rest => s :: flatSequenceOfStatements(rest)
        case Nil       => List()
      }

    override def visitReadCharStmt(
        ctx: OberonParser.ReadCharStmtContext
    ): Unit = {
      val varName = ctx.`var`.getText
      stmt = ReadCharStmt(varName)
    }

    override def visitReadLongRealStmt(
        ctx: OberonParser.ReadLongRealStmtContext
    ): Unit = {
      val varName = ctx.`var`.getText
      stmt = ReadLongRealStmt(varName)
    }

    override def visitReadRealStmt(
        ctx: OberonParser.ReadRealStmtContext
    ): Unit = {
      val varName = ctx.`var`.getText
      stmt = ReadRealStmt(varName)
    }

    override def visitReadLongIntStmt(
        ctx: OberonParser.ReadLongIntStmtContext
    ): Unit = {
      val varName = ctx.`var`.getText
      stmt = ReadLongIntStmt(varName)
    }

    override def visitReadIntStmt(
        ctx: OberonParser.ReadIntStmtContext
    ): Unit = {
      val varName = ctx.`var`.getText
      stmt = ReadIntStmt(varName)
    }

    override def visitReadShortIntStmt(
        ctx: OberonParser.ReadShortIntStmtContext
    ): Unit = {
      val varName = ctx.`var`.getText
    ...
    override def visitProcedureCall(
        ctx: OberonParser.ProcedureCallContext
    ): Unit = {
      val name = ctx.name.getText
      val args = new ListBuffer[Expression]
      val visitor = new ExpressionVisitor()
    ...
    override def visitIfElseIfStmt(
        ctx: OberonParser.IfElseIfStmtContext
    ): Unit = {
      val visitor = new ExpressionVisitor()
    ...
    /** {@inheritDoc }
      *
      * <p>The default implementation returns the result of calling {@link #
      * visitChildren} on {@code ctx}.</p>
      */
    override def visitForEachStmt(
        ctx: OberonParser.ForEachStmtContext
    ): Unit = {
    ...
    override def visitRepeatUntilStmt(
        ctx: OberonParser.RepeatUntilStmtContext
    ): Unit = {
      val visitor = new ExpressionVisitor()
    ...
    override def visitForRangeStmt(
        ctx: OberonParser.ForRangeStmtContext
    ): Unit = {
      val visitor = new ExpressionVisitor()
      ...
      // var := var + 1
      val accumulator = AssignmentStmt(
        VarAssignment(variable.name),
        AddExpression(variable, IntValue(1))
      )
  ...
  class AssignmentAlternativeVisitor extends OberonBaseVisitor[Unit] {
    ...
    override def visitVarAssignment(
        ctx: OberonParser.VarAssignmentContext
    ): Unit = {
      val varName = ctx.`var`.getText
      assignmentAlt = VarAssignment(varName)
    }

    override def visitArrayAssignment(
        ctx: OberonParser.ArrayAssignmentContext
    ): Unit = {
      val expressionVisitor = new ExpressionVisitor()
    ...
    override def visitRecordAssignment(
        ctx: OberonParser.RecordAssignmentContext
    ): Unit = {
      var expressionVisitor = new ExpressionVisitor()
    ...
    override def visitPointerAssignment(
        ctx: OberonParser.PointerAssignmentContext
    ): Unit = {
