...
class StandardLibrary[T](env: Environment[T]) {
  ...
  def abs = Procedure(
    "ABS",                             // name
    List(parameterByValue("x", IntegerType), parameterByReference("x", IntegerType)), // formal arguments
  ...
  def odd = Procedure(
    "ODD",
    List(parameterByValue("x", IntegerType), parameterByReference("x", IntegerType)),
    Some(BooleanType),
    List(),
    List(),
